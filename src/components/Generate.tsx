...
const Generate = () => {
  ...
  useEffect(() => {
    ...
    if (containerRef.current) {
      renderer.setSize(containerRef.current.clientWidth - 30, containerRef.current.clientHeight - 30);
    }
    ...
    // Create a cuboid that takes up the space of two division sizes
    createCuboid(divisionSize * 2, divisionSize , divisionSize , 4, 1);
    // createCuboid(divisionSize * 2, divisionSize , divisionSize , -2, -1);
    
    createCube(divisionSize, 5, 5)
    createCube(divisionSize, 1, 1)
  ...
  return (
    <div className="h-screen bg-[url(/src/assets/herobg.png)] flex gap-20 justify-center items-center">
      ...
      <div className="bg-black text-white p-6 rounded-lg shadow-md w-1/2 h-[70vh]" ref = {containerRef} style={{ width: '60%', height: '60%' }}>
